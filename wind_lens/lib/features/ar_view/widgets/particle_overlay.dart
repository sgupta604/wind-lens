...
import 'package:wind_lens/core/models/particle.dart';
...
import 'package:wind_lens/core/utils/wind_colors.dart';
...
///
/// Particles are rendered only in sky regions as determined by the [skyMask] ([SkyMaskData]).
...
class ParticleOverlay extends StatefulWidget {
  /// The sky mask data for filtering particles to sky region.
  ///
  /// Uses the Freezed [SkyMaskData] model which provides [isPointInSky]
  /// for normalized coordinate lookups and [skyFraction] for spawn rate.
  ...
  /// Optional high-frequency heading notifier from [SensorNotifiers].
  ///
  /// When provided, the tick loop reads heading from this notifier instead
  /// of [compassHeading]. This avoids full widget rebuilds at sensor rate
  /// (20-50Hz), letting heading flow directly to the particle update loop.
  ///
  /// If null, falls back to [compassHeading] (backward compatible).
  final ValueNotifier<double>? headingNotifier;

  /// Optional high-frequency pitch notifier from [SensorNotifiers].
  ///
  /// When provided, pitch data is available for future sky-aware
  /// particle placement. Currently reserved for Phase 3 integration.
  ///
  /// If null, pitch is not used in particle updates (backward compatible).
  ...
  ///
  /// When [headingNotifier] is provided, it takes precedence over
  /// [compassHeading] for high-frequency heading updates without
  /// widget rebuilds.
  ParticleOverlay({
    super.key,
...
class _ParticleOverlayState extends State<ParticleOverlay>
  ...
  /// Previous heading value for parallax calculation when using headingNotifier.
  ///
  /// When [headingNotifier] is provided, we track previous heading internally
  /// rather than relying on the widget property, since the heading is no
  /// longer passed through widget rebuilds.
  double _notifierPreviousHeading = 0.0;
  ...
  void _resetToSkyPosition(Particle p, SkyMaskData skyMask, {int maxAttempts = 10}) {
  ...
  void _onTick(Duration elapsed) {
    ...
    // Read heading: prefer notifier (high-frequency, no widget rebuild)
    // over widget property (fallback for backward compatibility).
    final double currentHeading;
    final double prevHeading;
    if (widget.headingNotifier != null) {
      currentHeading = widget.headingNotifier!.value;
      prevHeading = _notifierPreviousHeading;
      _notifierPreviousHeading = currentHeading;
    } else {
      currentHeading = widget.compassHeading;
      prevHeading = widget.previousHeading;
    }
    ...
    final compassRad = currentHeading * pi / 180;
    ...
    double headingDelta = currentHeading - prevHeading;
...
/// 1. Checks if the particle is in the sky region (via SkyMaskData)
...
class ParticleOverlayPainter extends CustomPainter {
  ...
  /// The sky mask data for filtering particles.
  final SkyMaskData skyMask;
